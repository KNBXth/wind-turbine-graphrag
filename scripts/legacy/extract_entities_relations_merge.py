...
# ========= 1. 路径配置 =========
INPUT_DIR = Path(r"E:\WORK\data\chunks\docs")
...
def extract_relations_from_sentence(sentence, entities):
    ...
    if faults and symptoms:
        if contains_any(sent, SYMPTOM_TRIGGERS):
            for f in faults:
                for s in symptoms:
                    if f["entity_name"] in sent and s["entity_name"] in sent:
    ...
    if faults and causes:
        if contains_any(sent, CAUSE_TRIGGERS):
            for f in faults:
                for c in causes:
                    if f["entity_name"] in sent and c["entity_name"] in sent:
            ...
            if contains_any(sent, weak_cause_hints):
                for f in faults:
                    for c in causes:
                        if f["entity_name"] in sent and c["entity_name"] in sent:
                            add_unique_relation(relations, seen, f, "CAUSED_BY", c)
...
for file_path in tqdm(chunk_files, desc="抽取实体关系-v3.1"):
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            page_span = f"{page_start}" if page_start == page_end else f"{page_start}-{page_end}"
